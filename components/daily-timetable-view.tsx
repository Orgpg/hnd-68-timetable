...
import { ChevronLeft, ChevronRight, BookOpen, Clock, GraduationCap, Sparkles, Calendar, Home, Sun } from "lucide-react"
...
export function DailyTimetableView() {
  ...
  const prevPeriod = () => {
    setCurrentPeriodIndex((prev) => (prev - 1 + timetable.length) % timetable.length)
  }
  ...
  return (
    <div className="space-y-6 sm:space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6" id="today">
        <DailySchedulePreview day={currentDay} sessions={todaySchedule} label="Today's Schedule" isToday={true} />
        <DailySchedulePreview day={tomorrowDay} sessions={tomorrowSchedule} label="Tomorrow's Schedule" />
      </div>

      {/* Day Navigator */}
      <Card
        className={`border-none shadow-lg ${isSelectedDayHoliday ? "bg-gradient-to-r from-orange-50 to-amber-50 dark:from-orange-950 dark:to-amber-950" : "bg-gradient-to-r from-teal-50 to-cyan-50 dark:from-teal-950 dark:to-cyan-950"}`} // Dark mode for day navigator card
        id="schedule"
      >
        <CardHeader className="p-4 sm:p-6">
          <div className="flex items-center justify-between">
            ...
            <CardTitle className="text-lg sm:text-2xl flex flex-col sm:flex-row items-center gap-2 sm:gap-3 text-center">
              ...
              <span
                className={`bg-gradient-to-r ${isSelectedDayHoliday ? "from-orange-600 to-amber-600" : "from-teal-600 to-cyan-600"} bg-clip-text text-transparent`}
              >
                {selectedDay} - {getDayName(selectedDay)}
              </span>
              {isSelectedDayHoliday && (
                <span className="text-sm bg-orange-100 text-orange-700 px-2 py-1 rounded-full">Holiday</span>
              )}
            </CardTitle>
            <Button
              variant="outline"
        ...
        <CardContent className="p-0">
          {selectedSchedule.length > 0 ? (
              ...
              {selectedSchedule.map((session, index) => {
                ...
                return (
                  <div
                    ...
                    } ${index === selectedSchedule.length - 1 ? "rounded-b-lg" : ""} transition-all duration-300 ${
                      isCompleted ? "opacity-60 grayscale-[50%]" : "hover:shadow-lg"
                    }`}
                  >
                    ...
                    <div className="flex items-center gap-2 sm:gap-3 mb-2 sm:mb-3">
                      ...
                      <span
                        className={`font-bold text-base sm:text-xl bg-white/20 px-3 sm:px-4 py-1 sm:py-2 rounded-full ${isCompleted ? "line-through" : ""}`}
                      >
                    ...
                    <h3
                      className={`font-bold text-lg sm:text-2xl mb-2 sm:mb-3 pr-12 sm:pr-16 leading-tight ${isCompleted ? "line-through" : ""}`}
                    >
                      {session.unit}
                    </h3>
                    ...
                    <div className="flex items-center gap-2 sm:gap-3">
                      ...
                      <span className={`text-sm sm:text-lg opacity-90 ${isCompleted ? "line-through" : ""}`}>
                        {session.teacher}
                      </span>
                    </div>
                  </div>
            ...
            <div className="text-center text-orange-600 dark:text-orange-400 py-12 sm:py-16 bg-gradient-to-br from-orange-50 to-amber-50 dark:from-orange-950 dark:to-amber-950">
              ...
              <h3 className="text-xl sm:text-2xl font-bold mb-2">Holiday - Rest Day</h3>
          ...
          return (
            <Button
              ...
              className={`h-16 sm:h-20 transition-all duration-300 border-none rounded-xl shadow-sm ${
                isCurrentDay ? "ring-2 ring-blue-400 ring-offset-2 dark:ring-blue-600 dark:ring-offset-gray-950" : "" // Dark mode for today ring
              } ${
                isHolidayDay
              ...
              } ${isSelected ? "shadow-lg scale-105 ring-2 ring-teal-400 dark:ring-teal-600" : "hover:scale-[1.02]"}`} // Dark mode for selected button ring
            >
              <div className="text-center">
                <div className="font-bold text-sm sm:text-lg">{day}</div>
                <div className="text-xs flex items-center justify-center gap-1">
                  {isHolidayDay ? <Home className="h-2 w-2 sm:h-3 w-3" /> : <BookOpen className="h-2 w-2 sm:h-3 w-3" />}
